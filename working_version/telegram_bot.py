...
def extract_random_segment(mp3_path, segment_length=60, output_path=None):
    """Вырезает случайный отрезок из MP3 файла."""
    ffmpeg_path = shutil.which('ffmpeg')
    if ffmpeg_path is None:
        return False
    
    duration = get_audio_duration(mp3_path)
    if duration is None or duration < segment_length:
        return False
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await message.reply_text("📥 Получен файл. Начинаю обработку...")
    ...
    try:
        ...
        if not os.path.exists(base_template):
            await status_msg.edit_text("❌ Ошибка: файл base.png не найден.")
            return
        
        # Этап 1: Создание обложки
        await status_msg.edit_text("🎨 Создаю обложку с пластинкой...")
        cover_path = os.path.join(temp_dir, "cover.png")
        if not create_album_cover(mp3_path, base_template, cover_path):
            await status_msg.edit_text("❌ Ошибка: не удалось создать обложку. Убедитесь, что в MP3 есть обложка.")
            return
        
        # Этап 2: Вырезание аудио отрезка
        await status_msg.edit_text("✂️ Вырезаю случайный 60-секундный отрезок...")
        ...
        if not extract_random_segment(mp3_path, segment_length=60, output_path=audio_path):
            await status_msg.edit_text("❌ Ошибка: не удалось вырезать аудио отрезок.")
            return
        
        # Этап 3: Создание видео для video note (квадратное, 640x640 - стандарт для video note)
        await status_msg.edit_text("🎬 Создаю круглое видеосообщение...")
        ...
        if not create_rotating_video(cover_path, audio_path, video_path, rotation_speed=1.0/3, size=640):
            await status_msg.edit_text("❌ Ошибка: не удалось создать видео.")
            return
                ...
                if 'streams' in probe_data and len(probe_data['streams']) > 0:
                    ...
                    if width != height:
                        ...
                        if not create_rotating_video(cover_path, audio_path, video_path, rotation_speed=1.0/3, size=512):
                            await status_msg.edit_text("❌ Ошибка: не удалось пересоздать квадратное видео.")
                            return
            except Exception as e:
        ...
        if file_size > 8:
            await status_msg.edit_text(f"❌ Ошибка: размер видео слишком большой ({file_size:.2f} МБ). Максимум 8 МБ для video note.")
            return
            ...
            try:
                ...
                if 'streams' in probe_data and len(probe_data['streams']) > 0:
                    ...
                    if width != height:
                        ...
                        await status_msg.edit_text(f"⚠️ Предупреждение: видео {width}x{height}, должно быть квадратным")
            except Exception as e:
                logger.warning(f"Не удалось проверить видео через ffprobe: {e}")
        
        # Отправляем результаты
        await status_msg.edit_text("✅ Обработка завершена! Отправляю результаты...")
        ...
        if response.status_code == 200:
            ...
            if result.get('ok'):
                ...
                if 'video_note' in message_result:
                    ...
                    await status_msg.edit_text("✅ Готово! Круглое видеосообщение отправлено.")

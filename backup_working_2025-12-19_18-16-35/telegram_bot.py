    ...
    try:
        ...
        try:
            ...
            if not os.path.exists(base_template):
                await query.edit_message_text("❌ Ошибка: файл base.png не найден.")
                return
            ...
            if not create_album_cover(mp3_path, base_template, cover_path, style=selected_style):
                await query.edit_message_text("❌ Ошибка: не удалось создать обложку. Убедитесь, что в MP3 есть обложка.")
                return
            ...
            if not extract_random_segment(mp3_path, segment_length=selected_duration, output_path=audio_path):
                await query.edit_message_text("❌ Ошибка: не удалось вырезать аудио отрезок.")
                return
            ...
            if not create_rotating_video(cover_path, audio_path, video_path, rotation_speed=selected_rotation, size=640):
                await query.edit_message_text("❌ Ошибка: не удалось создать видео.")
                return
            
            # Проверяем размер файла
            file_size = os.path.getsize(video_path) / (1024 * 1024)
            if file_size > 8:
                await query.edit_message_text(f"❌ Ошибка: размер видео слишком большой ({file_size:.2f} МБ). Максимум 8 МБ для video note.")
                return
            ...
            if response.status_code == 200:
                ...
                if result.get('ok'):
                    ...
                    await query.edit_message_text(f"❌ Ошибка API: {result.get('description', 'Unknown error')}")
            else:
                await query.edit_message_text(f"❌ HTTP ошибка: {response.status_code}")
